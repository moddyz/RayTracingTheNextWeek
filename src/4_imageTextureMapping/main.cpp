...
void PopulateSceneObjects( const gm::FloatRange& i_shutterRange, raytrace::SceneObjectPtrs& o_sceneObjects )
{
    ...
    o_sceneObjects.push_back( std::make_shared< raytrace::Sphere >( /* origin */ gm::Vec3f( 0, 0, 0 ),
                                                                    /* radius */ 2,
                                                                    earthMaterial ) );
}
...
int main( int i_argc, char** i_argv )
{
    ...
    cxxopts::Options options(
        "4_imageTextureMapping",
        "Ray tracing program which loads an image from disk to use as a texture for a material." );
