...
git checkout master
rm -rfv ./build
mkdir -p /tmp/RayTracingTheNextWeek
rm -rfv /tmp/RayTracingTheNextWeek
./build.sh /tmp/RayTracingTheNextWeek

# Checkout the gh-pages branch, remove all files, and copy the
# installed documentation into the repo root.
git checkout gh-pages
rm -rf *
cp -r /tmp/RayTracingTheNextWeek/docs/doxygen_docs/html/* ./
